...
function Page() { return <ProtectedRoute role="FARMER"><LayoutFarmer><Profile /></LayoutFarmer></ProtectedRoute>; }

function Profile() {
  const { user, refresh, logout } = useAuth();
  const navigate = useNavigate();
  const [form, setForm] = useState({ name: "", phone: "", city: "", propertyName: "", address: "" });
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (user) setForm({
      name: user.name ?? "", phone: user.phone ?? "", city: user.city ?? "",
      propertyName: user.propertyName ?? "", address: user.address ?? "",
    });
  }, [user]);

  const save = async () => {
    setLoading(true);
    try { await api.put("/me", form); await refresh(); toast.success("Perfil atualizado"); }
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <Card className="p-6">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2 sm:col-span-2"><Label>Nome da propriedade</Label><Input value={form.propertyName} onChange={(e) => setForm({ ...form, propertyName: e.target.value })} /></div>
          <div className="space-y-2 sm:col-span-2"><Label>Endereço para retirada</Label><Input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} /></div>
        </div>
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          <Button variant="ghost" className="text-destructive" onClick={() => { logout(); navigate({ to: "/login" }); }}>Sair</Button>
        </div>
      </Card>
    </div>
  );
